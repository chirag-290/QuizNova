...
import { Plus, Trash2, ArrowLeft, Search, Filter } from 'lucide-react';
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        navigate(`/exams/${res.data.data._id}`);
      }
    } catch (error) {
    ...
    <div className="font-sans bg-gradient-to-br from-[#0d1117] to-[#1f2937] text-white min-h-screen pb-12 animate-fade-in">
      ...
      <section className="relative py-12 px-6 overflow-hidden bg-[#1f1f1f] rounded-b-[3rem] shadow-xl mb-12">
        ...
        <div className="relative z-10 max-w-6xl mx-auto">
          <Link to="/exams" className="text-indigo-400 hover:text-indigo-300 flex items-center mb-4">
            <ArrowLeft className="h-5 w-5 mr-1" />
            <span>Back to Exams</span>
          </Link>
          <h1 className="text-4xl md:text-5xl font-extrabold tracking-tight mb-4">Create New Exam</h1>
          <p className="text-lg text-gray-300 max-w-3xl">
            Design your exam by selecting questions and configuring exam settings.
          </p>
        </div>
      </section>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-1 space-y-8">
              ...
              <div className="flex justify-center">
                ...
                >
                  {submitting ? <Spinner size="small" /> : 'Create Exam'}
                </button>
              </div>
            </div>
