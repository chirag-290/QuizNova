      ...
      try {
        ...
        if (user.role === "Student") {
          ...
          if (historyRes.data.success) {
            ...
            const passed = historyRes.data.data.filter(
              (item) => item.passed
            ).length;
            setStats((prev) => ({
              ...prev,
      ...
      <section className="relative py-16 px-6 overflow-hidden bg-[#1f1f1f] rounded-b-[3rem] shadow-xl mb-12">
        ...
        <div className="relative z-10 max-w-6xl mx-auto">
          ...
          <p className="text-lg text-gray-300 mb-6 max-w-3xl">
            {user.role === "Student" &&
              "Track your progress, explore new exams, and manage your certification journey."}
            {user.role === "Examiner" &&
              "Create exams, evaluate submissions, and monitor student performance."}
            {user.role === "Admin" &&
              "Oversee platform activities, manage users, and ensure system performance."}
          </p>
        </div>
      </section>
          ...
          {[
            ...
            user.role === "Student"
              ...
              : {
                  ...
                  value:
                    stats.examsTaken > 0
                      ? `${Math.round(
                          (stats.examsPassed / stats.examsTaken) * 100
                        )}%`
                      : "N/A",
                  color: "green",
                },
            user.role === "Admin"
              ...
              : {
                  ...
                  label: "Certificates",
                  value:
                    examHistory?.filter((exam) => exam.certificateGenerated)
                      .length || 0,
                  color: "blue",
                },
          ].map((stat, index) => (
            ...
            >
              <div className="flex items-center">
                <div
                  className={`p-3 rounded-full bg-${stat.color}-900/50 mr-4`}
                >
          ...
          <div className="flex justify-between items-center mb-8">
            ...
            <Link to="/exams">
              <Button
                variant="outline"
                size="small"
                className="border-pink-500 text-pink-400 hover:bg-pink-500/20 transition-colors"
              >
                View All
              </Button>
            </Link>
                ...
                >
                  <div className="p-6">
                    ...
                    <Link to={`/exams/${exam._id}`}>
                      <Button
                        size="small"
                        className="w-full bg-gradient-to-r from-indigo-600 to-pink-600 hover:from-indigo-500 hover:to-pink-500 text-white"
                      >
                        View Details
                      </Button>
                    </Link>
              ...
              <div className="overflow-x-auto bg-[#0f172a] rounded-xl">
                <table className="min-w-full">
                  <thead className="bg-[#1e293b] text-gray-300">
                    <tr>
                      {["Exam", "Score", "Status", "Date", "Certificate"].map(
                        (header) => (
                          <th
                            key={header}
                            className="py-3 px-6 text-left font-medium text-sm"
                          >
                            {header}
                          </th>
                        )
                    ...
                    {examHistory.map((item) => (
                      ...
                      >
                        <td className="py-4 px-6">
                          <Link
                            to={`/exams/${item.exam._id}/results`}
                        ...
                        <td className="py-4 px-6 text-gray-300">
                          {item.score}/{item.exam.totalMarks} (
                          {Math.round(
                            (item.score / item.exam.totalMarks) * 100
                          )}
                          %)
                        </td>
                        <td className="py-4 px-6">
                        ...
                        <td className="py-4 px-6">
                          {item.certificateGenerated ? (
                            <a
                              href={`https://quiznova.onrender.com${item.certificateUrl}`}
                              target="_blank"
                  ...
                  >
                    <div className="bg-[#1a2234] px-6 py-4 border-l-4 border-pink-500">
                      ...
                      <p className="text-sm text-pink-400">
                        {exam.pendingSubmissions.length} submissions pending
                        evaluation
                      </p>
                    </div>
                    <div className="p-4 space-y-3">
                      {exam.pendingSubmissions.slice(0, 3).map((submission) => (
                        ...
                        >
                          ...
                          <Link
                            to={`/exams/${exam.examId}/evaluate/${submission.submissionId}`}
                          >
                            <Button
                              size="small"
              ...
              ? [
                  {
                    icon: BookOpen,
                    title: "Browse Exams",
                    desc: "Explore all available exams and start your learning journey.",
                    to: "/exams",
                  },
                  {
                    icon: Clock,
                    title: "Exam History",
                    desc: "Review your past performance and download certificates.",
                    to: "/exam-history",
                  },
                  {
                    icon: Users,
                    title: "Update Profile",
                    desc: "Keep your information up to date for a better experience.",
                    to: "/profile",
                  },
                ].map((action, i) => (
              ...
              : [
                  {
                    icon: BookOpen,
                    title: "Create Exam",
                    desc: "Design a new exam with customizable questions and settings.",
                    to: "/exams/create",
                  },
                  {
                    icon: Brain,
                    title: "Add Question",
                    desc: "Expand your question bank with various question types.",
                    to: "/questions/create",
                  },
                  {
                    icon: CheckCircle,
                    title: "Evaluate",
                    desc: "Review and score pending exam submissions.",
                    to: "/evaluations",
                  },
                ].map((action, i) => (
                  <Link key={i} to={action.to} className="block">
